    ...
    def _update_config_from_packages(self):
        ...
        for package in self.packages:
            with zipfile.ZipFile(package, 'r') as package_zip:
                ...
                is_full = not pre_update.build_incremental
                if is_full:
                    if num_full_updates > 0:
                        raise RuntimeError("Expected at most a single full update zip!")
                    num_full_updates += 1
                else:
