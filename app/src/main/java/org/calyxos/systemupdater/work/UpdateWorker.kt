...
class UpdateWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    companion object {
        ...
        private const val PERIODIC_UPDATE_WORKER = "PERIODIC_UPDATE_WORKER"
        ...
        fun updateAutomatedCheck(context: Context, requiresBatteryNotLow: Boolean) {
            Log.i(TAG, "Updating periodic system updates!")
            runCatching {
                WorkManager.getInstance(context)
                .updateWork(buildUpdateWork(requiresBatteryNotLow))
            }.onFailure { Log.e(TAG, "Failed to update periodic system updates", it) }
        }

        private fun buildUpdateWork(requiresBatteryNotLow: Boolean = true): PeriodicWorkRequest {
            ...
            return PeriodicWorkRequestBuilder<UpdateWorker>(
                ...
                .setId(UUID.nameUUIDFromBytes(PERIODIC_UPDATE_WORKER.toByteArray()))
                .build()
        }
    }
