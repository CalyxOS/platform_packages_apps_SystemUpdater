...
class UpdateManagerImpl @Inject constructor(
    ...
    suspend fun checkUpdates(): Boolean {
        ...
        updateNotificationRepository.postNotification(UpdateStatus.CHECKING_FOR_UPDATE)
        return if (getUpdateConfig() != null) {
            Log.i(TAG, "New update available!")
            _updateStatus.value = UpdateStatus.UPDATE_AVAILABLE
            updateNotificationRepository.postNotification(UpdateStatus.UPDATE_AVAILABLE)
            true
        } else {
            Log.i(TAG, "No new update available!")
            _updateStatus.value = UpdateStatus.IDLE
            updateNotificationRepository.removeNotification(UpdateStatus.CHECKING_FOR_UPDATE)
            false
        }
    }
